...
1. _Name_ - Enter a descriptive name for your monitor, for example: "NIST Alerts Monitor".
2. _Score_ - CVE score filter in range `0` to `10`
3. _Domain_ - Regexp for `description` field match. If empty, any description will match. Expression example: `'(?i).*keycloak.*`.
4. _Cron_
